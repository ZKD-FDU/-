    ...
    return `<div class="view command-workspace">
      ...
      <section class="map-band command-band restored-command-band">
        <div class="terrain-panel command-terrain-panel three-ready" id="terrain-panel">
          ...
          <div class="command-milestones">${[['预警',m.config.warning_minute],['命令',m.config.evacuation_order_minute],['危险到达',m.config.danger_arrival_minute]].map(([label,t])=>`<button data-sand-time="${t}" ${!run?'disabled':''}>${label} ${t}′</button>`).join('')}<span>蓝白车辆：实际调度车队；积水带：路段水深状态</span></div>
        </div>
